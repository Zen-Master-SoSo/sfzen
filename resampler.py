...
class SampleResampler:
	...
	def __init__(self, sample, sample_rate, channels, bitdepth):
		self.sample = sample
		self.basedir = sample.basedir
		assert self.basedir is not None
		self.target_sample_rate = sample_rate
		self.target_channels = channels
		self.target_bitdepth = bitdepth
	...
	def needs_resample(self):
		return self.sample_rate != self.target_sample_rate \
	...
	def resample(self):
		...
		param_str = f'{self.target_sample_rate}-{self.target_channels}-{self.target_bitdepth}'
		loc_str = f'{self.offset}-{self.loop_start}-{self.loop_end}'
		out_file = join(tempdir(), f'{filetitle}-{param_str}-{loc_str}{ext}')
		if not out_file in self.converted_files:
			...
			logging.debug('Saved %s', out_file)
